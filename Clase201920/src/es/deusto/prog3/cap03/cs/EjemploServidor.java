...
public class EjemploServidor {
	...
	// Base de datos
	private static Connection conn;  // Conexión de Base de Datos
	private static Statement stat;   // Sentencia de Base de Datos
    
	public static void main(String[] args) {
		conn = BD.initBD( "ejemplo-cs.bd" ); // Inicializa la conexión sobre la BD
		stat = BD.usarCrearTablasBD( conn ); // Usa/crea la tabla de mensajes y crea una sentencia
	...
	public static class VentanaServidor extends JFrame {
	    ...
	    public void lanzaServidor() {
	    	...
	    	try {
	    		stat.close(); // Cierra la sentencia
	...
	private static class HiloComunicacion extends Thread {
		...
		public void run() {
	    	try {
    			...
    			BD.mensajeInsert( stat, "[CONEXIÓN]", nomCliente, System.currentTimeMillis() );
	    		...
	    		while(!finComunicacion) {  // Bucle de comunicación de tiempo real con el cliente
	    			try {
		    			...
		    			BD.mensajeInsert( stat, objRecibido.toString(), nomCliente, System.currentTimeMillis() );
    			...
    			// Guarda la desconexión en base de datos
    			BD.mensajeInsert( stat, "[DESCONEXIÓN]", nomCliente, System.currentTimeMillis() );
	    		socket.close();
